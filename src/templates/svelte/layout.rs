...
pub fn generate(
  ...
  style_import = style_import.replace("NAME_LOWER", name);

  responsive = responsive.replace("NAME_LOWER", &name_lower);
  responsive = responsive.replace("NAME", name);

  let layout_path = format!("{path}/+layout.svelte");
  let styles_path = format!("{path_ui}/styles/{name}.layout.styles{ext}");
  let responsive_path = format!("{path_ui}/styles/{name}.layout.styles.responsive{ext}");
